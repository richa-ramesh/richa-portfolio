...
type ExperienceFilter = 'all' | 'fulltime' | 'internships';
...
export default function ResumeDownload({ onClose }: ResumeDownloadProps) {
  ...
  const filteredExperiences = experience.filter(exp => {
    ...
    if (expFilter === 'fulltime') return exp.category === 'industry';
    if (expFilter === 'internships') return exp.category === 'internship';
    return true;
  });
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-5xl bg-[#0b0f19] md:rounded-3xl border border-slate-800/80 shadow-2xl overflow-hidden my-4 print:my-0 print:border-none print:shadow-none print:bg-white print:text-black">
        ...
        <div className="bg-[#030409]/95 px-6 py-5 border-b border-slate-900 sticky top-0 z-20 backdrop-blur-md print:hidden space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-[10px] font-mono text-slate-400 uppercase tracking-widest mt-0.5">
                  Click skills below to trace experience benchmarks
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="cursor-pointer inline-flex items-center space-x-1.5 bg-slate-950 border border-slate-900 hover:border-slate-800 px-3 py-2 rounded-xl text-xs font-mono text-zinc-350 transition-all duration-300"
                title="Copy flat resume text to clipboard"
              >
                {copied ? <Check size={13} className="text-teal-400 animate-scale" /> : <Copy size={13} className="text-rose-400" />}
                <span>{copied ? 'Copied Content' : 'Copy Plain Text'}</span>
              </button>

              {/* Print action */}
              <button
                onClick={handlePrint}
                className="cursor-pointer inline-flex items-center space-x-1.5 bg-gradient-to-r from-rose-600 to-purple-600 hover:from-rose-500 hover:to-purple-500 text-white px-4 py-2 rounded-xl text-xs font-mono font-semibold transition-all duration-300 shadow-md shadow-rose-500/20 hover:scale-[1.02]"
              ...
              <button
                onClick={onClose}
                className="cursor-pointer w-8 h-8 rounded-xl bg-slate-950 border border-slate-900 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-all"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-3 border-t border-slate-900">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-900 grow">
                <button
                  onClick={() => setExpFilter('all')}
                  className={`text-[10px] font-mono py-1 px-2.5 rounded text-center grow cursor-pointer transition-all ${
                    expFilter === 'all'
                      ? 'bg-slate-900 border border-slate-800 text-rose-400 font-bold'
                ...
                <button
                  onClick={() => setExpFilter('fulltime')}
                  className={`text-[10px] font-mono py-1 px-2.5 rounded text-center grow cursor-pointer transition-all ${
                    expFilter === 'fulltime'
                      ? 'bg-slate-900 border border-slate-800 text-rose-400 font-bold'
                      : 'text-slate-400 hover:text-white'
                  }`}
                  title="Show Full-time software engineering roles"
                >
                  SWE Track
                </button>
                <button
                  onClick={() => setExpFilter('internships')}
                  className={`text-[10px] font-mono py-1 px-2.5 rounded text-center grow cursor-pointer transition-all ${
                    expFilter === 'internships'
                      ? 'bg-slate-900 border border-slate-800 text-rose-400 font-bold'
                      : 'text-slate-400 hover:text-white'
                  }`}
                  title="Show internship software positions"
                >
                  ML & Intern
                </button>
              </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex bg-slate-950 p-1 rounded-lg border border-slate-900 grow">
                <button
                  ...
                  className={`text-[10px] font-mono py-1 px-2.5 rounded text-center grow cursor-pointer transition-all ${
                    previewTheme === 'light'
                      ? 'bg-[#000000]/10 border border-rose-500/10 text-rose-400 font-bold'
                      : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Crisp A4 Light
                ...
                <button
                  ...
                  className={`text-[10px] font-mono py-1 px-2.5 rounded text-center grow cursor-pointer transition-all ${
                    previewTheme === 'dark'
                      ? 'bg-[#000000]/10 border border-rose-550/20 text-rose-455 font-bold'
                      : 'text-slate-400 hover:text-white'
                  }`}
                >
                  Console Dark
          ...
          {selectedSkill && (
            <div className="bg-[#030409] border border-rose-500/20 text-xs px-3 py-1.5 rounded-lg flex items-center justify-between text-rose-350 font-mono animate-fade-in">
              ...
              <button
                ...
                className="hover:text-white cursor-pointer px-1.5 py-0.5 rounded bg-rose-500/10 hover:bg-rose-500/30 text-[10px] transition-colors"
              >
                Clear Highlight [X]
              </button>
            </div>
        ...
        <div
          className={`transition-colors duration-300 px-6 py-8 md:p-12 font-sans overflow-x-auto ${
            previewTheme === 'light'
              ...
              : 'bg-[#0f0d0b] text-slate-200 border border-rose-950/20'
        ...
        >
          ...
          <div className="min-w-[690px] max-w-[800px] mx-auto print:min-w-0 print:max-w-none">
            ...
            <div className={`text-center ${density === 'compact' ? 'space-y-1 pb-3' : 'space-y-2 pb-5'} border-b-2 border-slate-900`}>
              <h1 className={`${density === 'compact' ? 'text-2xl' : 'text-3xl'} font-extrabold tracking-tight font-display ${previewTheme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                {basics.name}
              </h1>
              <p className={`text-xs font-mono font-bold uppercase tracking-widest ${previewTheme === 'light' ? 'text-zinc-600' : 'text-rose-500'}`}>
                {basics.title}
              </p>
              ...
              <div className="flex justify-center space-x-6 text-[11px] font-mono mt-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center space-x-1 text-zinc-655 hover:text-rose-500 transition-colors font-bold"
                >
                  <Linkedin size={11} />
            ...
            {/* SUMMARY PARAGRAPH */}
            <div className={`${density === 'compact' ? 'py-2.5' : 'py-4'} border-b border-zinc-200/80`}>
              ...
              <div className="space-y-2 text-xs">
                {skills.map((s, idx) => (
                  <div key={idx} className="grid grid-cols-4 gap-2 items-center">
                    <span className={`font-bold ${previewTheme === 'light' ? 'text-zinc-700' : 'text-slate-350'}`}>
                      {s.category}:
                    </span>
                        ...
                        return (
                          <button
                            ...
                            className={`cursor-pointer text-[10px] px-2 py-0.5 rounded font-mono transition-all ${
                              isHighlighted
                                ? 'bg-pink-500 text-white font-bold ring-2 ring-pink-400/50'
                                : previewTheme === 'light'
                                  ? 'bg-zinc-100 hover:bg-zinc-200 text-zinc-800 border'
                                  : 'bg-stone-900 hover:bg-zinc-800 text-slate-300 border border-stone-800'
                            }`}
                    ...
                    <div
                      ...
                      className={`space-y-1 my-2 transition-all duration-300 rounded-lg ${
                        selectedSkill && hasMatchesInThisCompany
                          ? previewTheme === 'light' 
                            ? 'bg-rose-50/50 p-2 border-l-2 border-rose-450'
                            : 'bg-rose-950/20 p-2 border-l-2 border-rose-400'
                          : 'border-l border-zinc-250'
                      }`}
                    >
                      <div className="flex justify-between text-xs font-bold leading-none">
                        ...
                        <span className={`font-mono text-[11px] ${previewTheme === 'light' ? 'text-zinc-600' : 'text-rose-400'}`}>
                          {exp.dates}
                        </span>
                      </div>
                      ...
                      <div className="flex justify-between text-[10px] text-zinc-500 font-mono mt-0.5">
                        ...
                        <span className={`capitalize px-1.5 py-0.2 rounded text-[9px] font-semibold border ${
                          previewTheme === 'light'
                            ? 'bg-[#ffffff] text-zinc-650 border-zinc-200'
                          ...
                          return (
                            <li
                              ...
                              className={`leading-relaxed transition-all duration-300 ${
                                hasHighlight
                                  ? 'bg-rose-100/80 text-zinc-950 font-medium px-1.5 rounded ring-2 ring-rose-400/30'
                                  : previewTheme === 'light'
                                    ? 'text-zinc-700'
                                    : 'text-slate-450'
                              }`}
            ...
            <div className={`${density === 'compact' ? 'py-2.5' : 'py-4'} border-b border-zinc-200/80`}>
              <h2 className={`text-xs font-mono font-bold uppercase tracking-wider ${density === 'compact' ? 'mb-1.5' : 'mb-2.5'} ${previewTheme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                Scholarly Publications
              </h2>
              <ul className={`list-disc pl-4 text-xs ${density === 'compact' ? 'space-y-0.5' : 'space-y-1.5'}`}>
                  ...
                  return (
                    <li
                      ...
                      className={`leading-relaxed ${
                        hasHighlight
                          ? 'bg-rose-100 text-zinc-950 font-medium px-1 rounded ring-1 ring-rose-400/20'
                          : previewTheme === 'light'
                            ? 'text-zinc-750'
                            : 'text-slate-350'
                      }`}
            ...
            <div className={`${density === 'compact' ? 'py-2.5' : 'py-4'} pb-0`}>
              <h2 className={`text-xs font-mono font-bold uppercase tracking-wider ${density === 'compact' ? 'mb-1.5' : 'mb-3'} ${previewTheme === 'light' ? 'text-slate-900' : 'text-white'}`}>
                Education Chronology
              </h2>
                  ...
                  return (
                    <div
                      ...
                      className={`flex justify-between items-start text-xs rounded transition-all duration-300 p-1 ${
                        hasHighlight
                          ? 'bg-rose-100 text-zinc-950 ring-1 ring-rose-400/20'
                          : ''
                      }`}
                    >
                      ...
                      <div className="text-right font-mono text-[11px] space-y-0.5">
                        <span className={previewTheme === 'light' ? 'text-zinc-650' : 'text-slate-400'}>
                           {edu.dates}
                        </span>
                        {edu.gpa && (
                          <span className="block text-rose-650 font-bold font-mono">
                            GPA: {edu.gpa}
                          </span>
                        )}
                      </div>
        ...
        <div className="bg-[#030409]/95 px-6 py-4 border-t border-slate-900 text-[10px] text-center text-slate-500 font-mono flex flex-col sm:flex-row sm:justify-between items-center justify-center gap-2 print:hidden w-full">
          <span>Richa Rameshkrishna • Systems & ML Engineer Portfolio CV</span>
          <span className="text-rose-500">Available for full-time opportunities</span>
        </div>    </div>  </div>
      </div>
    </div>
  );
